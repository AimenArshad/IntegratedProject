...
export default function UpdateProfile() {
  ...
  const [password,setpassword] =useState(0);
    ...
    useEffect(()=>{
      (async ()=>{
        Axios.get('http://localhost:5000/api/residents/get/3600000000').then((response)=> {
          setResident(response.data);
          
          });})();},[]);

     const updateUser=()=>{
            Axios.put(`http://localhost:5000/api/residents/update/${resident.cnic}`,{password:password,fname: fname,lname:lname,age:age,email:email});
